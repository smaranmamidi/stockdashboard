...
def create_category_sales_chart(filtered_df):
    ...
    df_category = filtered_df.groupby('Category')['Sales'].sum().sort_values(ascending=False)
    
    fig = px.bar(
        x=df_category.index,
        y=df_category.values,
...
def create_top_products_chart(filtered_df, top_n=10):
    ...
    df_products = filtered_df.groupby('Product Name')['Sales'].sum().nlargest(top_n)
    
    fig = px.barh(
        x=df_products.values,
        y=df_products.index,
        orientation='h',
        title=f'Top {top_n} Products by Sales',
        labels={'x': 'Sales ($)', 'y': 'Product'},
        template='plotly_white',
        color=df_products.values,
...
def create_segment_sales_chart(filtered_df):
    ...
    df_segment = filtered_df.groupby('Segment')['Sales'].sum().sort_values(ascending=False)
